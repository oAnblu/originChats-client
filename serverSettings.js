...
function openServerSettings() {
    ...
    if (serverSettingsBtn && state.serverUrl !== 'dms.mistium.com') {
    ...
    rolesList.innerHTML = serverSettingsState.roles.map(role => {
        ...
        return `
            <div class="role-item" data-role="${role.name}">
                <div class="role-color-preview" style="background: ${role.color || '#5865F2'}"></div>
                <div class="role-info">
                    <div class="role-name" style="color: ${role.color || '#5865F2'}">${role.name}</div>
                    <div class="role-description">${role.description || 'No description'}</div>
                </div>
                <div class="role-actions">
                    ${!isSystemRole ? `
                        <button class="role-action-btn" onclick="openEditRoleModal('${role.name}')" title="Edit Role">
                            <i data-lucide="edit-2"></i>
                        </button>
                        <button class="role-action-btn delete" onclick="deleteRole('${role.name}')" title="Delete Role">
...
function renderManagementChannels() {
    ...
    channelsList.innerHTML = serverSettingsState.channels.map((channel, index) => {
        ...
        return `
            <div class="channel-management-item" data-channel="${channel.name}">
                <div class="channel-type-icon">
                    <i data-lucide="${icon}"></i>
                </div>
                <div class="channel-info">
                    <div class="channel-name">${channel.display_name || channel.name} ${channel.type === 'separator' ? '(separator)' : ''}</div>
                    <div class="channel-type">${channel.type}</div>
                </div>
                <div class="channel-management-actions">
                    ${channel.type !== 'separator' ? `
                        <button class="channel-action-btn" onclick="moveChannelUp('${channel.name}', ${index})" title="Move Up" ${index === 0 ? 'disabled' : ''}>
                            <i data-lucide="chevron-up"></i>
                        </button>
                        <button class="channel-action-btn" onclick="moveChannelDown('${channel.name}', ${index})" title="Move Down" ${index === serverSettingsState.channels.length - 1 ? 'disabled' : ''}>
                            <i data-lucide="chevron-down"></i>
                        </button>
                        <button class="channel-action-btn" onclick="openEditChannelModal('${channel.name}')" title="Edit Channel">
                            <i data-lucide="edit-2"></i>
                        </button>
                        <button class="channel-action-btn delete" onclick="deleteChannel('${channel.name}')" title="Delete Channel">
                            <i data-lucide="trash-2"></i>
                        </button>
                    ` : `
                        <button class="channel-action-btn" onclick="openEditChannelModal('${channel.name}')" title="Edit Separator">
                            <i data-lucide="edit-2"></i>
                        </button>
                        <button class="channel-action-btn delete" onclick="deleteChannel('${channel.name}')" title="Delete Separator">
                            <i data-lucide="trash-2"></i>
                        </button>
                    `}
                </div>
...
function renderManagementMembers(filter = '') {
    ...
    membersList.innerHTML = sortedMembers.map(member => `
        <div class="member-row" data-member="${member.username}">
            <div class="member-avatar">
                <img src="${getAvatarUrl(member.username)}" alt="${member.username}">
            </div>
            <div class="member-details">
                <div class="member-name">${member.username}</div>
                <div class="member-roles-display">
                    ${(member.roles || []).slice(0, 3).map(role => `
                        <span class="member-role-badge" style="background: ${getRoleColor(role)}">${role}</span>
                    `).join('')}
                    ${(member.roles || []).length > 3 ? `<span class="member-role-badge">+${member.roles.length - 3}</span>` : ''}
                </div>
            </div>
            <div class="member-actions">
                <button class="member-action-btn" onclick="openMemberRolesModal('${member.username}')" title="Manage Roles">
...
function openMemberRolesModal(username) {
    ...
    title.textContent = 'Manage Member Roles';
    ...
    memberInfo.innerHTML = `
        ...
        <div class="member-info-details">
            <h3>${username}</h3>
            <span class="member-info-username">${(member.roles || []).join(', ') || 'No roles'}</span>
        </div>
    `;
...
function renderAvailableRoles() {
    ...
    rolesList.innerHTML = serverSettingsState.roles.map(role => `
        <div class="available-role-item ${currentRoles.includes(role.name) ? 'selected' : ''}" data-role="${role.name}" onclick="toggleRoleSelection('${role.name}')">
            <div class="role-checkbox">
                <i data-lucide="check"></i>
            </div>
            <div class="role-color-preview" style="width: 24px; height: 24px; background: ${role.color || '#5865F2'}"></div>
            <span class="available-role-name" style="color: ${role.color || '#5865F2'}">${role.name}</span>
        </div>
    `).join('');
